...
\ugh{For historic reasons, modeling and simulation techniques are often applied reactively and later in the development process.}
\ugh{Reasons include that OEMs (original equipment manufacturers) have acted mainly as systems integrators, ordering and integrating pre-allocated functionality to ECU (electronic control unit) hardware.}
\magnus{The underlined could be dropped for space concerns}
...
Leveraging these potentially large benefits requires change throughout the \emph{automotive ecosystem} (see sidebar \ref{bar:ecosys}). 
This paper presents benefits, enablers, and impediments to adoption by relevant stakeholders in the ecosystem; information useful when articulating a value proposition for overcoming  resistance to change. \magnus{Here could be a good place for a technology adoption reference}
...
To further investigate the ecosystem for virtucal verification we held semi-structured interviews with a large North American OEM, its suppliers, and tool vendors.
...
\magnus{Should the method be concretized and expanded on further?}
